...
let connectionState = {
  ...
  serverId: `user-server-${Date.now()}`, // Unique identifier for this server
...
const getConnectionOptions = () => ({
  ...
  appName: 'LandManagementSystem-User',
...
const setupConnectionEventHandlers = () => {
  ...
  mongoose.connection.on('connected', () => {
    ...
    if (connectionMetrics.averageConnectionTime === 0) {
...
const connectWithRetry = async () => {
  ...
  try {
    ...
    console.log(`🔄 Connection attempt ${connectionState.connectionAttempts} (User Server)`);
